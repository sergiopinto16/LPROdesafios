...
public class State {
	...
	public void equal2(int x, int y, String c) {
		...
		for (int j = 0; j < ListD.size(); j++) { // para cada drag�o
			d1 = ListD.get(j);
			d1.setMove(false);

			if (this.m1.nextIsWall(x, y, h1, e1)) { // Se for parede, j� n�o verifica os proximos dragoes pq n�o depende
													// de d1
				Imprimir = "Ha Parede";
				j = ListD.size();
	...
	public void movDragon() {
		...
		for (int j = 0; j < ListD.size(); j++) {
			...
			} else if (d1.DragonIsInCornerBL(d1)) {// verifica se est� no canto BOTTOM LEFT
				System.out.println(num);
				switch (num) {
				case 2:
			...
			} else if (d1.DragonIsInCornerTR(d1)) {// verifica se est� no canto TOP RIGHT
				System.out.println(num);
				switch (num) {
				case 1:
			...
			} else if (d1.DragonIsInCornerBR(d1)) {// verifica se est� no canto BOTTOM RIGHT
				System.out.println(num);
				switch (num) {
				case 2:
					if (m1.CorrectMoveDragon(h1, e1, d1, s1, 1)) {
